...
// @version      0.1
// @updateURL    https://raw.githubusercontent.com/Airistotal/UserScripts/main/Scripts/PrettyPrintJson.js
// @downloadURL  https://raw.githubusercontent.com/Airistotal/UserScripts/main/Scripts/PrettyPrintJson.js
// @description  Pretty prints json objects in pre tags
// @author       You
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function() {
  ...
  for (let i of document.getElementsByTagName("pre")) {
    var data = {};
    if (IsJsonString(i.innerText, data))
    {
      i.innerHTML = '';

      var code = document.createElement("code");
      code.innerHTML = JSON.stringify(data, undefined, 2);
      i.appendChild(code);
    }
  }
...
function IsJsonString(str, output) {
    try {
      Object.assign(output, JSON.parse(str));
    } catch (e) {
        return false;
    }
    return true;
...
function applyStyling() {
  var backgroundColor = '#251318';
  var baseColor = '#E7EFC8';
  var propertyColor = '#76A29E';
  var valueColor = '#C1BFEE';
  var color2 = "#B6BBC9";
  var color3 = "#BEC18F";

  for(let i of document.getElementsByClassName("hljs")) {
    i.setAttribute("style", "display: block;overflow-x: auto;padding: 0.5em;background: "+backgroundColor+";color: "+baseColor+";");
  }

  for(let i of document.getElementsByClassName("hljs-subst")) {
    i.setAttribute("style", "color: "+baseColor+";");
  }

  for(let i of document.getElementsByClassName("hljs-comment")) {
    i.setAttribute("style", "color: #888888;font-weight: bold;");
  }

  for(let i of document.getElementsByClassName("hljs-meta")) {
    i.setAttribute("style", "color: #1f7199;");
  }

  for(let i of document.getElementsByClassName("hljs-meta-string")) {
    i.setAttribute("style", "color: #4d99bf;");
  }

  for(let i of document.getElementsByClassName("hljs-literal")) {
    i.setAttribute("style", "color: #78A960;");
  }

  for(let i of document.getElementsByClassName("hljs-emphasis")) {
    i.setAttribute("style", "font-style: italic;");
  }

  var coloredBold = getElementsByClassNames(
    "hljs-title",
    "hljs-attr",
    "hljs-section");

  for (let i of coloredBold) {
    i.setAttribute("style", "color: "+propertyColor+";font-weight: bold;");
  }

  var bolded = getElementsByClassNames("hljs-selector-tag","hljs-meta-keyword","hljs-doctag","hljs-name","hljs-strong");

  for(let i of bolded) {
    i.setAttribute("style", "font-weight: bold;");
  }

  var colored = getElementsByClassNames("hljs-type","hljs-string","hljs-number","hljs-selector-id","hljs-selector-class","hljs-quote","hljs-template-tag","hljs-deletion");

  for (let i of colored) {
    i.setAttribute("style", "color: "+valueColor+";")
  }

  var colored2 = getElementsByClassNames("hljs-regexp","hljs-symbol","hljs-variable","hljs-template-variable","hljs-link","hljs-selector-attr","hljs-selector-pseudo");

  for (let i of colored2) {
    i.setAttribute("style", "color: "+color2+";")
  }

  var colored3 = getElementsByClassNames("hljs-built_in","hljs-bullet","hljs-code","hljs-addition");

  for (let i of colored3) {
    i.setAttribute("style", "color: "+color3+";");
  }
}

function getElementsByClassNames() {
  var elems = new Array();

  for (i = 0; i < arguments.length; i++) {
    var className = arguments[i];

    elems = elems.concat(Array.from(document.getElementsByClassName(className)));
  }
